...
Versioning is handled by the `Versioning` GitHub Actions workflow. Select the `core` package and choose `current` to publish the version already declared in `packages/core/package.json`, or a SemVer bump for later releases. New tags use the package-scoped format `iam-sdk@X.Y.Z`.
